...
type Storage interface {
	...
	SaveMsgSentToBotUser(email string, msgUid int64, botUserUid int64) error
	// Проверяем было ли сообщение уже отправленно пользователю бота
	MsgWasSentToBotUser(email string, msgUid int64, botUserUid int64) (bool, error)
	//
	SaveMsgInfo(email string, msg *email.MessageEnvelope) error
	GetMsgFromAddress(email string, msgUid int64) (string, error)
	ShutDown() error
}
